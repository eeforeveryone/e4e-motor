...
#include "TX_EN.h"
    ...
    #define TX_EN_SetP4PinDriveMode(shift, mode)  \
    do { \
        TX_EN_PC =   (TX_EN_PC & \
                                (uint32)(~(uint32)(TX_EN_DRIVE_MODE_IND_MASK << \
                                (TX_EN_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (TX_EN_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define TX_EN_SetP4PinDriveMode(shift, mode)  \
        do { \
            TX_EN_USBIO_CTRL_REG = (TX_EN_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(TX_EN_DRIVE_MODE_IND_MASK << \
                                    (TX_EN_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (TX_EN_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		TX_EN_SetP4PinDriveMode(TX_EN__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(TX_EN_DR & (uint8)(~TX_EN_MASK));
    drVal = (drVal | ((uint8)(value << TX_EN_SHIFT) & TX_EN_MASK));
    TX_EN_DR = (uint32)drVal;
}
...
{
    return (uint8)((TX_EN_PS & TX_EN_MASK) >> TX_EN_SHIFT);
}
...
{
    return (uint8)((TX_EN_DR & TX_EN_MASK) >> TX_EN_SHIFT);
}
...
void TX_EN_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  TX_EN_INTCFG & (uint32)(~(uint32)position);
    TX_EN_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(TX_EN_INTSTAT & TX_EN_MASK);
	TX_EN_INTSTAT = maskedStatus;
    return maskedStatus >> TX_EN_SHIFT;
}
