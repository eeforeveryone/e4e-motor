...
void EEmotor__setPIParams(struct EEmotor *self, float sI, float sP, float pI, float pP){ //update PID values
    self->PosReg.K_I = pI;
    self->PosReg.K_P = pP;
    self->speedReg.K_I = sI;
    self->speedReg.K_P = sP;
}
